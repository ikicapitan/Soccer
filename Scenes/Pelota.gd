...
func _physics_process(delta):
	...
	if(pase): #Si esta en proceso de pase (fue pateado para un pase)
		if(get_node("AnimationPlayer").current_animation_position <= get_node("AnimationPlayer").get_animation("mov_der").length / 2):
			$Sprite.scale+= delta * dim_maxima / (get_node("AnimationPlayer").get_animation("mov_der").length / 2)
			$Sprite.position+= delta * posl_m / (get_node("AnimationPlayer").get_animation("mov_der").length / 2)
		else:
			$Sprite.scale-= delta * dim_maxima / (get_node("AnimationPlayer").get_animation("mov_der").length / 2)
			$Sprite.position-= delta * posl_m / (get_node("AnimationPlayer").get_animation("mov_der").length / 2)
